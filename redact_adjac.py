...
def is_Station_in_Line(station1,linestring1):
    ...
    return station1.longitude+0.01>min(temp_x) and station1.longitude-0.01<max(temp_x) and station1.width+0.01>min(temp_y) and station1.width-0.01<max(temp_y)\
...
if __name__ == '__main__':
    ...
    for row in excel_file:
        if row[0].value != 'name' and row[0].value != None:
            temp = Station(row[0].value, row[1].value, row[2].value, row[3].value, row[4].value, row[5].value,
                        row[6].value, row[7].value)
            if temp.history!= None:
                main_list_station.append(temp)
    ...
    for x in main_linestring_list:
        print (x.output())
        ws_result.append({'A': x.init_station.x, 'B': x.init_station.y, 'C': x.finish_station.x, 'D': x.finish_station.y, 'E': x.output()})
